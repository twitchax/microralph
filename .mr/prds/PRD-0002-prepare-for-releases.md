...
tags: [release, ci, crates.io, github-releases, cargo-make, semver, changelog]
...
- [ ] `cargo make codecov` runs successfully and generates coverage.lcov
- [ ] CI workflow uploads codecov results to Codecov
- [ ] `cargo make build-linux` produces target/x86_64-unknown-linux-gnu/release/mr
- [ ] `cargo make build-macos` produces target/aarch64-apple-darwin/release/mr
- [ ] `cargo make build-windows` produces target/x86_64-pc-windows-gnu/release/mr.exe
- [ ] `cargo make build-wasm` produces target/wasm32-wasip2/release/mr.wasm
- [ ] CI workflow uploads all platform artifacts on main branch
- [ ] `cargo make changelog` generates CHANGELOG.md
- [ ] `cargo make release` performs version bump via cargo-release
- [ ] `cargo make publish-crates` publishes to crates.io (dry-run test)
- [ ] `cargo make publish-github` creates GitHub release with binaries (dry-run test)
