...
You are running `mr bootstrap --reconstruct` on an existing repository that has development history but no PRDs. Your goal is to create PRDs that represent the major milestones and features that have already been completed.
...
- **Notes**: This PRD was automatically generated by `mr bootstrap --reconstruct`
```
